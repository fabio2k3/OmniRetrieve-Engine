...
class TestIsValid:
    ...
    def test_no_question_mark(self):
        ok, reason = _is_valid("How attention mechanisms work", CHUNK)
        assert ok is False
        assert "?" in reason
...
class TestQueryGenerator:
    def _mock_ollama(self, response):
        mock = MagicMock()
    ...
    def test_returns_none_when_ollama_missing(self):
        ...
        with patch.dict("sys.modules", {"ollama": None}):
            assert gen.generate(CHUNK) is None

    def test_returns_none_when_always_invalid(self):
    ...
    def test_truncates_long_chunk(self):
        ...
        prompt = ollama.chat.call_args[1]["messages"][0]["content"]
