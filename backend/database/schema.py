...
schema.py
=========
Esquema SQLite unificado para OmniRetrieve-Engine.
...
Módulo indexing (índice invertido)
    terms       — vocabulario: una fila por token único
    postings    — índice invertido: frecuencia cruda por (término, documento)
...
CREATE TABLE IF NOT EXISTS documents (
    ...
    index_error      TEXT,
    indexed_tfidf_at TEXT           -- timestamp de indexación TF (indexer); NULL = pendiente
);
...
-- ── Módulo de indexación (índice invertido TF) ──────────────────────────────
